...
@interface LXQRCodeScanManager()<AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
@property (nonatomic, strong) AVCaptureSession *session;
@property (nonatomic, strong) AVCaptureVideoDataOutput *videoDataOutput;
@property (nonatomic, strong) AVCaptureVideoPreviewLayer *videoPreviewLayer;
@property (nonatomic, strong) GetBrightnessBlock brightnessBlock;
@property (nonatomic, strong) ScanBlock scanBlock;


@end
@implementation LXQRCodeScanManager

+(instancetype)sharedManager{
    
    static LXQRCodeScanManager *manager = nil;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager  =[[LXQRCodeScanManager alloc]init];
    });
    return manager;
}
...
- (void)setupSessionPreset:(NSString *)sessionPreset metadataObjectTypes:(NSArray *)metadataObjectTypes currentController:(UIViewController *)currentController {
    ...
    if (sessionPreset == nil || metadataObjectTypes== nil || currentController == nil) {
        NSException *excp = [NSException exceptionWithName:LXDisplayName reason:@"setupSessionPreset:metadataObjectTypes:currentController: 方法中的 sessionPreset 参数不能为空" userInfo:nil];
        [excp raise];
    }
